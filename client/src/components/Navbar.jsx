...
function Navbar() {
  ...
  const logout = async () => {
    try {
      ...
      const { data } = await axios.post(backendUrl + '/api/auth/logout'); // Fixed missing "/"
    ...
    <div className='w-full flex justify-between items-center p-4 sm:p-6 sm:px-24 absolute top-0'>
      ...
      {userData ? (
        <div className='relative group'>
          <div className='w-8 h-8 flex items-center justify-center rounded-full bg-black text-white cursor-pointer'>
            {userData.name[0].toUpperCase()}
          </div>
          <div className='absolute invisible group-hover:visible top-full right-0 z-10 text-black rounded pt-2'>
